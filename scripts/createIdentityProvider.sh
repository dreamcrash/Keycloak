...
if [[ $# -ne 5 ]]; then
	...
	echo "Usage: $0 <Keycloak IP> <Admin User Name> <Admin Password> <Realm Name> <IDP Json Data>">&2
        exit 1
fi
...
curl -k -sS -X POST "https://$KEYCLOAK_IP/auth/admin/realms/$REALM_NAME/identity-provider/instances" \
